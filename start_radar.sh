...
LOG_DIR="${LOG_DIR:-/tmp/radar2026_startup_$(date +%Y%m%d_%H%M%S)}"
START_DELAY="${START_DELAY:-2}"
LIVOX_DRIVER_DELAY="${LIVOX_DRIVER_DELAY:-3}"
...
if [[ "$SKIP_CALIBRATION" == "1" ]]; then
    ...
    run_background "01_calibrate_radar" "$START_DELAY" \
    ...
    if [[ "$DRY_RUN" == "1" ]]; then
        ...
        read -r -p "     标定完成后回到此终端按 Enter，脚本会关闭标定节点并继续启动: "
        stop_one "$CALIBRATION_PID" "01_calibrate_radar"
    fi
...
run_background "02_serial_driver" "$START_DELAY" \
    ros2 launch rm_serial_driver serial_driver.launch.py \
    "port:=$SERIAL_PORT" "force_color:=$FORCE_COLOR"

run_background "03_livox_driver" "$LIVOX_DRIVER_DELAY" \
    ros2 launch livox_ros2_driver livox_lidar_launch.py

run_background "04_lidar_registration" "$START_DELAY" \
    ros2 launch dynamic_cloud lidar.launch.py

run_background "05_radar" "$START_DELAY" \
    ros2 launch tdt_vision radar.launch.py

run_background "06_debug_map" "$START_DELAY" \
    ros2 run debug_map debug_map

run_background "07_auto_record" 0 \
