    ...
    : Node("radar_detect_node", node_options)
{
    ...
    debug_pub = this->create_publisher<sensor_msgs::msg::CompressedImage>("detect_debug_image", 10);
...
void Detect::callback(const std::shared_ptr<sensor_msgs::msg::Image> msg)
{
    ...
    RCLCPP_DEBUG(this->get_logger(), "Detecting image.");
    ...
    if (result.size() == 0) {
        RCLCPP_DEBUG(this->get_logger(), "No Car!");
        // // 显示原图
        // cv::Mat final_img;
        ...
        RCLCPP_DEBUG(this->get_logger(), "Too Many Car!");
        return;
    }
    ...
    if (!has_armor) {
        RCLCPP_DEBUG(this->get_logger(), "No Armor!");
        return;
    }
    ...
    std::chrono::duration<double> time_used =
        std::chrono::duration_cast<std::chrono::duration<double>>(end -
                                                                  begin);
    RCLCPP_DEBUG(this->get_logger(), "Detect Time: %.3f ms", time_used.count() * 1000.0);
    // cv::Mat final_img;
    // cv::resize(img, final_img, cv::Size(1536, 1125));
    // cv::imshow("detect", final_img);
    ...
    // }
    if (debug) {
        // 使用 cv_bridge 将画好框的 OpenCV Mat 转换回 ROS Image 消息并发布
        auto debug_msg = cv_bridge::CvImage(msg->header, "bgr8", img).toCompressedImageMsg();
