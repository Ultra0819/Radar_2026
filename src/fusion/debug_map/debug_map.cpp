...
public:
        ...
        : Node("debug_map", options)
    {
        ...
        robust_detect_pub = 
            this->create_publisher<vision_interface::msg::DetectResult>("/robust_detect", 10);
            
        // 2. 创建一个 10Hz 的定时器，保证即使传感器卡顿，串口端依然有数据发送
        timer_ = this->create_wall_timer(
            std::chrono::milliseconds(100),
            std::bind(&DebugMap::timer_callback, this));        
    ...
    rclcpp::Publisher<vision_interface::msg::DetectResult>::SharedPtr robust_detect_pub;
    rclcpp::TimerBase::SharedPtr timer_;

    void timer_callback()
    {
        vision_interface::msg::DetectResult robust_msg;
        robust_msg.header.stamp = this->now();
        robust_msg.header.frame_id = "rm_frame";
        ...
        for (int i = 0; i < 6; i++) {
            // 设置 1.0 秒为存活阈值。如果在 1 秒内有雷达或视觉更新，则输出坐标
            // 否则，严格输出 0 坐标，确保元素不缺失
            if (current_time - blue_update[i] < 1.0) {
                robust_msg.blue_x[i] = blue_point[i].x;
                robust_msg.blue_y[i] = blue_point[i].y;
            } else {
                robust_msg.blue_x[i] = 0.0f;
                robust_msg.blue_y[i] = 0.0f;
            }

            if (current_time - red_update[i] < 1.0) {
                robust_msg.red_x[i] = red_point[i].x;
                robust_msg.red_y[i] = red_point[i].y;
            } else {
                robust_msg.red_x[i] = 0.0f;
                robust_msg.red_y[i] = 0.0f;
            }
        }
        
        // 发布融合后的兜底数据
        robust_detect_pub->publish(robust_msg);
    }
